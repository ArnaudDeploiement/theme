...
class admin_setting_markdown extends admin_setting {
    ...
    public function output_html($data, $query = '') {
        ...
        if (file_exists("{$CFG->dirroot}/course/format/customsession/{$this->filename}")) {
            $filecontents = file_get_contents($CFG->dirroot . '/course/format/customsession/' . $this->filename);
        } else {
            $filecontents = 'Custom Session format admin_setting_markdown -> file not found: ' . $this->filename;
        }
        $context->markdown = format_text($filecontents, FORMAT_MARKDOWN);

        return $OUTPUT->render_from_template('format_customsession/customsession_admin_setting_markdown', $context);
    }
}
