...
namespace format_customsession;
...
class admin_setting_information extends admin_setting {
    ...
    public function output_html($data, $query = '') {
        ...
        if (!empty($formats['customsession'])) {
            $plugininfo = $formats['customsession'];
        } else {
            $plugininfo = \core_plugin_manager::instance()->get_plugin_info('format_customsession');
        ...
        if (!empty($plugininfo->maturity)) {
            switch ($plugininfo->maturity) {
                case MATURITY_ALPHA:
                    $context['maturity'] = get_string('versionalpha', 'format_customsession');
                    $context['maturityalert'] = 'danger';
                    break;
                case MATURITY_BETA:
                    $context['maturity'] = get_string('versionbeta', 'format_customsession');
                    $context['maturityalert'] = 'danger';
                    break;
                case MATURITY_RC:
                    $context['maturity'] = get_string('versionrc', 'format_customsession');
                    $context['maturityalert'] = 'warning';
                    break;
                case MATURITY_STABLE:
                    $context['maturity'] = get_string('versionstable', 'format_customsession');
        ...
        if ($CFG->branch != $this->mbranch) {
            ...
            $context['versioncheck'] .= '<a href="https://moodle.org/plugins/format_customsession" target="_blank">Moodle.org</a>.  ';
        ...
        return $OUTPUT->render_from_template('format_customsession/customsession_admin_setting_information', $context);
    }
}
