...
class format_customsession extends core_courseformat\base {
    ...
    public function get_default_section_name($section) {
        ...
        if ($section->sectionnum == 0) {
            return get_string('section0name', 'format_customsession');
        }

        $lastsectionnumber = $this->get_last_section_number();
        if ($section->sectionnum == $lastsectionnumber) {
            return get_string('lastsectionname', 'format_customsession');
        }

        return get_string('newsection', 'format_customsession');
    }
    ...
    public function course_format_options($foreditform = false) {
        ...
        if ($foreditform && !isset($courseformatoptions['training_button_url']['label'])) {

            $courseformatoptionsedit = [
                'training_button_url' => [
                    'label' => new lang_string('setting_training_button_url_name', 'format_customsession'),
                    'element_type' => 'text',
                    'help' => 'setting_training_button_url_help',
                    'help_component' => 'format_customsession',
                ],
                'finished_button_url' => [
                    'label' => new lang_string('setting_finished_button_url_name', 'format_customsession'),
                    'element_type' => 'text',
                    'help' => 'setting_finished_button_url_help',
                    'help_component' => 'format_customsession',
                ],
            ];
            ...
            foreach ($this->settings as $settingname => $settingvalue) {
                if (isset($settingvalue)) {
                    ...
                    if (
                        ...
                        $this->settings[$settingname] = get_config('format_customsession', 'default' . $settingname);
                    }
                }
            }
        }
 ...
 */
function format_customsession_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
    require_once($CFG->dirroot . '/course/lib.php');
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            'SELECT s.* FROM {course_sections} s JOIN {course} c ON s.course = c.id WHERE s.id = ? AND c.format = ?',
            [$itemid, 'customsession'],
            MUST_EXIST
        );
